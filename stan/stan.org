...
\item \ty{-M} mutation rate of the target regions, $\theta/\mbox{nucleotide}$
\end{enumerate}
#+end_export
#+begin_src go <<Declare options, Pr. \ref{pr:sta}>>=
  ...
  optR := flag.String("r", "4501-5500", "target regions")
  optM := flag.Float64("m", 0.01, "background mutation rate, " +
	  "theta per nucleotide")
  optMM := flag.Float64("M", -0.1, "target region mutation " +
...
#+end_src
#+begin_export latex
If the end of a target region extends beyond the end of the sequence,
we bail with message.
#+end_export
#+begin_src go <<Check region, Pr. \ref{pr:sta}>>=
  if e > *optL {
	  log.Fatalf("Region %d-%d extends beyond " +
...
perhaps print it, and simulate the sequences along it. If the target
mutation rate is negative, we delete the target regions---from the
neighbors, of course.
#+end_export
...
#+end_export
#+begin_src go <<Functions, Pr. \ref{pr:sta}>>=
  func genPartCoal(np, n int, b string, ran *rand.Rand) *nwk.Node {
	  var root *nwk.Node
...
$n$ of these are the leaves, which we label by appending consecutive
numbers to the label base. The last leaf is the root.
#+end_export
...
same as for the targets. However, the mutation rate in the target
...
target region, we draw a random number out of \ty{ts} target sites and
...
variables: the number of target and background sites, the number of
...
#+end_src
#+begin_export latex
For the target position map we iterate over the target regions and map
...
variable, and the two maps of target and background positions.
#+end_export
...
#+end_export
#+begin_src go <<Functions, Pr. \ref{pr:sta}>>=
  func genHaps(v *nwk.Node, haps *Haplotypes, t2p,
	  b2p map[int]int) {
	  if v == nil { return }
...
columns, a name like \ty{t1} followed by a string of zeros and
ones. We print this haplotype table with a \ty{tabwriter}. We
construct the \ty{tabwrite}, print the target and neighbor haplotypes
to it, and flush it.
#+end_export
...
#+end_src
#+begin_export latex
We iterate over the positions and print them in a single row.
#+end_export
...
#+begin_src go <<Print neighbor haplotypes, Pr. \ref{pr:sta}>>=
  for j := haps.tn; j < haps.tn+haps.nn; j++ {
	  fmt.Fprintf(w, "n%d\t", j - haps.tn + 1)
	  for i := 0; i < len(haps.hap); i++ {
		  c := strconv.Itoa(int(haps.hap[i][j]))
		  fmt.Fprintf(w, "%s", c)
	  }
...
delete them from the neighbor sequences accordingly by copying the
spacers in between. After the deletion we mark the new end of the
neighbor sequences.
#+end_export
...
just created to be overwritten.
#+end_export
