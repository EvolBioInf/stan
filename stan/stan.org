...
#+end_src
#+begin_export latex
We also declare four options to administer runs of \ty{stan}:
\begin{enumerate}
\item \ty{-p} print coalescent tree
...
#+end_export
#+begin_src go <<Declare options, Pr. \ref{pr:sta}>>=
  optP := flag.Bool("p", false, "print coalescent tree")
...
neighbor coalescent is \ty{nil}, so we don't need to change anything
before traversing the neighbor tree.
#+end_export
...
haplotypes, construct the actual haplotypes, and convert them to DNA
sequences.
#+end_export
...
#+begin_src go <<Map leaves to haplotypes, Pr. \ref{pr:sta}>>=
  ...
  start := 0
  tc.Sib = nil
  start = mapLeaves(tc, start, leaf2hap)
  tc.Sib = nc
  start = mapLeaves(nc, start, leaf2hap)
#+end_src
#+begin_export latex
...
#+end_export
#+begin_src go <<Functions, Pr. \ref{pr:sta}>>=
  func mapLeaves(v *nwk.Node, h int, l2h map[int]int) int {
	  if v == nil {
		  return h
	  }
	  if v.Child == nil {
	  ...
	  h = mapLeaves(v.Child, h, l2h)
	  h = mapLeaves(v.Sib, h, l2h)
	  return h
  }
#+end_src
#+begin_export latex
...
#+begin_src go <<Iterate over target mutations, Pr. \ref{pr:sta}>>=
  ...
  for i := 0; i < m; i++ {
	  ...
	  ss := make([]byte, n)
	  //<<Record segregating sites, Pr. \ref{pr:sta}>>
...
descendants.
#+end_export
#+begin_src go <<Record segregating sites, Pr. \ref{pr:sta}>>=
  if v.Child != nil {
