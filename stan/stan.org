...
#+begin_src go <<Declare options, Pr. \ref{pr:sta}>>=
  ...
  optMM := flag.Float64("M", 0.25, "targe region mutation " +
	  "rate, theta per nucleotide; " +
...
target mutation rate differ. The sibling link of the root of the
neighbor coalescent is already \ty{nil}, so we don't need to change
anything before traversing the neighbor tree.
#+end_export
...
#+end_src
#+begin_export latex
We print the haplotypes with a \ty{tabwriter}. We construct the
\ty{tabwrite}, print the target and neighbor haplotypes to it, and
flush it.
#+end_export
