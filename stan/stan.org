...
$\np$, the total sample size, $n$, the prefix of each leaf label, and
the random number generator. Then we coalesce the two partial
coalescents into the final coalescent. We determine the branch lengths
of that tree and determine the mutations along its branches based on
their lengths.
#+end_export
...
coalescents. Specifically, the root's child link points to the
neighbor coalescent, while the neighbor coalescent's sibling link
points to the target coalescent. Figure~\ref{fig:top} shows the
topology around the root we have thus just created.
\begin{figure}
  \begin{center}
    \psset{nodesep=2pt,rowsep=0.5cm,colsep=0.5cm}
    \begin{psmatrix}
      &&\circlenode{root}{\ty{root}}\\
      &\circlenode{nc}{\ty{nc}} & & \circlenode{tc}{\ty{tc}}\\
      \Rnode{c1}{} && \Rnode{c2}{}
      \ncline{->}{root}{nc}
      \ncline{->}{nc}{tc}
      \ncline{->}{nc}{c1}
    ...
    \end{psmatrix}
  \end{center}
  \caption{The topology around the root of the final coalescent
    consisting of the neighbors coalescent, \ty{nc}, and the targets
    coalescent, \ty{tc}.}\label{fig:top}
\end{figure}
#+end_export
...
regions differs.  Note that we need to isolate the neighbor root from
its sibling the target root before traversing the neighbor coalescent
(see Figure~\ref{fig:top}). After the traversal we restore the sibling
link.
#+end_export
